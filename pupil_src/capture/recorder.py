...
class Recorder(Plugin):
    ...
    def __init__(self, g_pool, session_name=get_auto_name(), rec_dir=None,
                 user_info={'name': '', 'additional_field': 'change_me'}, info_menu_conf={}, show_info_menu=False,
                 record_eye=False, audio_src='No Audio', raw_jpeg=True):
    ...
    def init_gui(self):
        ...
        self.menu.append(ui.Selector('raw_jpeg', self, selection=[True, False],
                                     labels=["bigger file, less CPU", "smaller file, more CPU"], label='Compression'))
    ...
    def toggle(self, _=None):
        if self.running:
            ...
            self.notify_all(
                {'subject': 'recording.should_start', 'session_name': self.session_name, 'remote_notify': 'all'})
    ...
    def start(self):
        ...
        self.notify_all({'subject': 'recording.started', 'rec_path': self.rec_path, 'session_name': self.session_name,
                         'record_eye': self.record_eye, 'compression': self.raw_jpeg})
    ...
    def open_info_menu(self):
        ...
        populate_info_menu()
        self.info_menu.append(ui.Info_Text(
        ...
        try:
            with open(self.meta_info_path, 'a') as csvfile:
                csv_utils.write_key_value_file(csvfile, {
                    ...
                    'World Camera Resolution': str(self.g_pool.capture.frame_size[0]) + "x" + str(
