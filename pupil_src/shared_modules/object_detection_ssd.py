...
class Object_Detection_SSD(Plugin):
    ...
    def __init__(self, g_pool, detect=False,  ratio=0.5, webcam=False):
    ...
    def init_gui(self):
        ...
        '''
        self.menu = ui.Growing_Menu('Object Detection SSD')
        self.g_pool.sidebar.append(self.menu)

        self.button = ui.Thumb('detect', self, label='OD', hotkey='o')
    ...
    def update_gui_markers(self):
        ...
        self.menu.append(ui.Info_Text('This plugin uses the SSD Algorithm implementation for object Detection'))
        self.menu.append(ui.Slider('ratio', self, step=0.05, min=0.1, max=1))
        self.menu.append(ui.Switch('detect', self, label='detector'))
        self.menu.append(ui.Switch('webcam', self, label='webcam'))
    ...
    def get_init_dict(self):
        return {'mode': self.mode, 'detect': self.detect,  'ratio': self.ratio, 'webcam':self.webcam}
    ...
    def update(self, frame, events, thr=0.5):
        ...
        if self.running and self.detect:
    ...
    def cleanup(self):
        ...
        """
        self.notify_all({'subject':'object_detection_ssd.stopped'})
        if self.menu:
